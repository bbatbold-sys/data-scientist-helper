...
import { GitMerge, Eye, Plus, ArrowRight } from 'lucide-react'
import toast from 'react-hot-toast'
import { mergeDatasets, previewMerge } from '../../api/client'
...
export default function MergeWorkspace() {
  ...
  const params: MergeParams = { left_id: leftId, right_id: rightId, left_key: leftKey, right_key: rightKey, how, new_name: newName }

  const handlePreview = async () => {
    if (!leftId || !rightId || !leftKey || !rightKey) {
      toast.error('Please select both datasets and join keys')
      return
    }
    setLoading('preview')
    try {
      const result = await previewMerge(params)
      setPreview(result)
    } catch (e: unknown) {
  ...
  const handleMerge = async () => {
    ...
    setLoading('merge')
    try {
      const result = await mergeDatasets(params)
      addDataset(result)
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-80 flex-shrink-0 border-r border-slate-200 dark:border-slate-800 overflow-y-auto scrollbar-thin p-4 bg-white dark:bg-slate-900">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Merge Datasets</h2>
        </div>

        {datasets.length < 2 && (
          <div className="text-center py-8 px-4">
            <Plus size={28} className="text-slate-300 dark:text-slate-600 mx-auto mb-2" />
            <p className="text-sm text-slate-400 dark:text-slate-500">Upload at least 2 datasets to merge</p>
          </div>
        )}
        ...
        <div className="flex-1 overflow-auto scrollbar-thin scrollbar-thumb-slate p-4">
          {!preview ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-slate-400 dark:text-slate-500 text-sm">Configure merge settings and click Preview</p>
            </div>
          ) : (
